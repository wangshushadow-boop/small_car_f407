...
本文档说明 `rpi_host` 目录中的模块。正式算法通过 ROS2 bridge 访问底盘；
`CarClient`、命令行和传感器监视器作为 bridge 的传输层及独立诊断入口。
...
| 方向 | 建议 |
| --- | --- |
| 算法接入 | 订阅标准 ROS2 话题，不直接调用 `CarClient` 或操作串口。 |
| 视觉算法 | 使用 ROS2 相机驱动和 `image_transport`；现有工具仅用于抓图诊断。 |
| 音频算法 | 独立语音节点输出意图，不让音频处理阻塞底盘 bridge。 |
| 实时控制 | ROS2 发布 `/cmd_vel`，MCU 继续承担手柄优先级、超时和避障保护。 |
