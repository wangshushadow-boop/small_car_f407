...
| 2 | USB 手柄 | 检测到摇杆或按键操作时立即接管，回中 2 秒后释放控制权。 |
| 3 | 上位机 | 通过串口3协议 v3 接收 `mm/s` 和 `mrad/s` 物理速度。 |
| 4 | 空闲 | 没有有效输入时，底盘停车。 |
...
| 扩展项 | 建议 |
| --- | --- |
| 上位机控制 | 已由 `raspi_link.c` 实现物理速度协议，并通过 `RaspiLink_GetControlCommand()` 接入控制仲裁。 |
| 闭环速度控制 | 保持 ROS 和协议物理量不变，在 `chassis_auto.c` 后增加基于编码器反馈的独立控制层。 |
